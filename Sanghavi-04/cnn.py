...
class CNN(object):
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        """
         ...
         :return: None
         """
        dense_add = layers.Dense(num_nodes, activation=activation, name=name, trainable=trainable)
        self.model.add(dense_add)
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        """
         ...
         :return: Layer object
         """
        layer_obj2d = self.model.add(keras.layers.Conv2D(filters=num_of_filters, kernel_size=kernel_size, padding=padding, strides=strides, activation=activation, name=name, trainable=trainable))
        return layer_obj2d
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        """
         ...
         :return: Layer object
         """
        layer_objmax2d = self.model.add(keras.layers.MaxPool2D(pool_size=pool_size, strides=strides, padding=padding, name=name))
        return layer_objmax2d
    ...
    def set_weights_without_biases(self,weights,layer_number=None,layer_name=""):
        ...
        if layer_name:
            ...
            if layer_number >= 0 and layer_name:
    ...
    def remove_last_layer(self):
        ...
        """
        remove_layer = self.model.pop()
        #self.model.summary(self.model.layers)
    ...
    def load_a_model(self,model_name="",model_file_name=""):
        ...
        if model_name:
            if model_name == 'VGG16':
                #self.model = keras.applications.VGG16().layers()
                self.model = keras.models.Sequential(tf.keras.applications.VGG19().layers)
            elif model_name == 'VGG19':
                #self.model = keras.applications.VGG19().layers()
                self.model = keras.models.Sequential(tf.keras.applications.VGG19().layers)
            ...
            #self.model = keras.models.load_model(model_file_name).layers()
    ...
    def save_model(self,model_file_name=""):
        ...
        :return: model
        """
        s_model = self.model.save(model_file_name)
        return s_model
    ...
    def predict(self, X):
        ...
        """
        op_tensor = self.model.predict(X.astype(float))
        return op_tensor
    ...
    def evaluate(self,X,y):
        """
         ...
         """
        loss_matric = self.model.evaluate(x=X, y=y)
        return loss_matric 
    ...
    def train(self, X_train, y_train, batch_size, num_epochs):
        ...
        loss_values= self.model.fit(x=X_train, y=y_train, batch_size=batch_size, epochs=num_epochs, verbose=1)
        return loss_values.loss_values['loss']
